...
### 2.3 获取会话详情
- **Endpoint**: `GET /api/v1/ai/sessions/{session_id}`
- **响应**: `SessionResponse`

### 2.4 删除会话
- **Endpoint**: `DELETE /api/v1/ai/sessions/{session_id}`
- **响应**: `{"message": "Session deleted successfully"}`

### 2.5 发送消息 (支持流式)
- **Endpoint**: `POST /api/v1/ai/sessions/{session_id}/messages`
- **协议**: 支持 SSE (Server-Sent Events) 和普通响应
- **请求体**:
  ```json
  {
    "content": "文本内容",
  ...
  ```
- **响应**: 
  - 流式模式 (`stream: true`): 持续推送 SSE 数据包
    ```
    data: {"content": "xxx", "finish_reason": null}
    data: {"content": "!", "finish_reason": "stop"}
    data: [DONE]
    ```
  - 非流式模式 (`stream: false`): `MessageResponse`

### 2.6 获取会话消息列表
- **Endpoint**: `GET /api/v1/ai/sessions/{session_id}/messages`
- **参数**:
  - `page`: 页码，默认 1
...
### 2.7 添加消息反馈
- **Endpoint**: `POST /api/v1/ai/sessions/{session_id}/feedback`
- **参数**:
  - `message_id`: 消息 ID
- **请求体**:
  ```json
...
- **游客限制规则**：游客默认每日可发起 3 个新会话。**[!] 技术实现分层**：为了应对高并发下的配额检查压强，**系统采用"Redis 实时热路径拦截，DB 离线冷路径存根"的单真源策略**。
  ...
  - `POST /api/v1/ai/sessions/{session_id}/messages` 接口需在校验等级限额前，先校验 `attachments` 参数
...
---
- [x] 对齐代码实现：端点路径已与 `app/api/v1/ai/router.py` 同步。
- [x] 新增端点：获取会话详情、消息列表、反馈、提供商列表。
